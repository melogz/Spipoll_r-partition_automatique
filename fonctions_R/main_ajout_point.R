...
main_ajout_point_collection<- function(fichier){
  df<- fichier
  df <- df[!duplicated(paste(df$lat,df$long,df$date_de_session)),]
  point_sur_carte <- carte_a_point(df,carte,titre)
  plot(point_sur_carte)
  write.table(df, file = paste0(getwd(),"/data_entree/data_SPIPOLL/df_collection.csv"),sep =';', row.names = FALSE)
  return (df)
}
...
main_ajout_point_observation<- function(fichier,carte,debut,fin,chemin_enregistrement,titre='point présent'){
  ...
  point_sur_carte <- carte_a_point(df,carte,titre)
  plot(point_sur_carte)
  write.table(df, file = paste0(getwd(),"/data_entree/data_SPIPOLL/df_observé.csv"),sep =';', row.names = FALSE)
  return (df)
}
